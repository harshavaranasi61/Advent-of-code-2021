...
while(1):
    try:
        status,coordinates=[x for x in input().split()]
        x,y,z=coordinates.split(',')
        x1,x2=[int(f) for f in x[2:].split('..')]
        y1,y2=[int(f) for f in y[2:].split('..')]
        z1,z2=[int(f) for f in z[2:].split('..')]
        commands.append((status=="on",[x1,x2,y1,y2,z1,z2]))
    except:
        break
